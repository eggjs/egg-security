...
const allowTypeList = [ 'all', 'referer', 'ctoken' ];

exports.allowTypeList = allowTypeList;

/**
 * check csrf type
 * @param {String} type the csrf type
 */
exports.checkCsrfType = type => {
  const shouldCheckAll = type === allowTypeList[0];
  return {
    isLegalType: allowTypeList.includes(type),
    shouldCheckAll,
    shouldCheckOrigin: shouldCheckAll || type === allowTypeList[1],
    shouldCheckCtoken: shouldCheckAll || type === allowTypeList[2],
  };
};
...
exports.preprocessConfig = function(config) {
  ...
  // Make sure originWhiteList is case insensitive
  if (config.csrf && config.csrf.originWhiteList) {
    config.csrf.originWhiteList = config.csrf.originWhiteList.map(ref => ref.toLowerCase());
  }
